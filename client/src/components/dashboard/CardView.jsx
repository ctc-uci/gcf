...
const CardView = ({ data, openEditForm }) => {
  return (
    ...
    >
      {data.map((p) => (
        <GridItem key={p.id}>
          <Card w="auto" minW={324} minH={400} h="auto" br={20}>
            ...
            <CardBody position="relative">
              ...
              <Box position="absolute" bottom={15} right={2}>
                <Badge p={2} borderRadius="full" bg="#808080" color="white">
                  {p.students} Students
                </Badge>
              </Box>
            </CardBody>
            <CardFooter bg="gray.200" w="100%" h="auto" minh="20%">
              <VStack align="left">
                <Text> {p.title} </Text>
                <Flex gap={3} flexWrap="wrap">
                  {p.programDirectors.map((d) => (
                    <Text key={d.userId} fontSize="sm">
