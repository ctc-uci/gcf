...
function ProgramTable({ role = "admin", userId }) {
  ...
  const { unorderedData, setUnorderedData, handleSort } = useTableSort(setPrograms);
...
// set search query as value entered in search bar
 ...
 useEffect(() => {
    function filterPrograms(search) {
      ...
      const filtered = unorderedData.filter(program => 
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        setUnorderedData(rows.map(mapRow));
      } catch (err) {
